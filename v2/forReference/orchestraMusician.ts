...
namespace OrchestraMusician {
    ...
    export function microbitOrchestraHero(playerNumber: number) {
        gameActivated = true
        radio.setGroup(84)
        ...
        radio.onReceivedString(function(name: string) {    
            if (name == "A") {
                ...
                led.plot(2,4)
                transmitScore(playerNumber, myScore)
            }
        })
...
//
    ...
    function transmitScore(playerN: number, score: number){
        basic.pause(10 * playerN)
        scoreIsLocked = true
        radio.setGroup(84)
        radio.sendValue("S" + playerN, score)
        //radio.setGroup(83)
